...
ai_clients = {
    ...
    'qwen': OpenAI(
        ...
        # 默认为新加坡地域的模型 如北京改成https://dashscope.aliyuncs.com/compatible-mode/v1
    )
}
...
def calculate_intelligent_position(signal_data, price_data, current_position):
    ...
    if not config.get('enable_intelligent_position', True):
        fixed_contracts = 0.5  # 固定仓位大小，可以根据需要调整
...
def analyze_with_deepseek(price_data):
    ...
    2. **风险管理** (权重10%)：考虑持仓、盈亏状况和止损位置
    3. **趋势跟随**: 明确趋势出现时立即行动，不要过度等待
    4. **信号明确性**:
    - 强势上涨趋势 → BUY信号
    - 强势下跌趋势 → SELL信号  
    ...
    try:
        # 获取当前选择的AI提供商
        ai_provider = TRADE_CONFIG.get('ai_provider', 'deepseek')
